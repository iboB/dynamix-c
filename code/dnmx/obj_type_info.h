...
// for faster acciess
typedef struct dnmx_obj_type_info_call_table_msg {
...
} dnmx_obj_type_info_call_table_msg;

typedef struct dnmx_obj_type_info_call_table_entry {
    ...
    dnmx_obj_type_info_call_table_msg top_bid_msg;
    ...
    dnmx_obj_type_info_call_table_msg* begin;
    dnmx_obj_type_info_call_table_msg* end;
} dnmx_obj_type_info_call_table_entry;

typedef struct dnmx_obj_type_info {
    const dnmx_domain* domain; // domain of this type info

    dnmx_obj_type_info_call_table_entry* call_table;
    uint32_t call_table_size;

    // a buffer for all dynamically additional message pointers used in the call table
    dnmx_obj_type_info_call_table_msg* message_data_buffer;
...
bool dnmx_obj_type_info_has_mixin(const dnmx_obj_type_info* info, dnmx_mixin_id id);
inline bool dnmx_obj_type_info_has_mixin(const dnmx_obj_type_info* info, dnmx_mixin_id id) {
